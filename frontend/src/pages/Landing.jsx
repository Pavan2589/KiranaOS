...
export default function Landing() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:'Nunito, sans-serif' }}>
      ...
      {/* Nav */}
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 48px', height:64, borderBottom:'1px solid #E8F5E9', position:'sticky', top:0, background:'rgba(255,255,255,0.97)', backdropFilter:'blur(8px)', zIndex:100 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:34, height:34, borderRadius:10, background:THEME.gradient, display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, boxShadow:'0 2px 8px rgba(12,131,31,0.3)' }}>🏪</div>
          <div>
            <div style={{ fontWeight:900, fontSize:20, color:THEME.primary, lineHeight:1, letterSpacing:-0.5 }}>KiranaOS</div>
            <div style={{ fontSize:9, color:THEME.accent, fontWeight:700, letterSpacing:1.5, textTransform:'uppercase' }}>hyperlocal delivery</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {user ? (
            <>
              ...
              <button onClick={() => navigate(`/${user.role}`)} style={{ background:THEME.gradient, color:'#fff', border:'none', borderRadius:8, padding:'8px 18px', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'Nunito,sans-serif', boxShadow:'0 2px 8px rgba(12,131,31,0.3)' }}>Dashboard</button>
            ...
            <>
              ...
              <button onClick={() => navigate('/auth/customer')} style={{ background:THEME.gradient, color:'#fff', border:'none', borderRadius:8, padding:'8px 18px', fontWeight:800, fontSize:13, cursor:'pointer', fontFamily:'Nunito,sans-serif', boxShadow:'0 2px 8px rgba(12,131,31,0.3)' }}>Get Started →</button>
            </>
          )}
        </div>
      </nav>
        ...
        <div style={{ position:'relative', zIndex:1, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:48, maxWidth:1100, margin:'0 auto' }}>
          <div style={{ flex:1, minWidth:280 }}>
            ...
            <h1 style={{ fontSize:52, fontWeight:900, color:'#fff', lineHeight:1.1, marginBottom:18, letterSpacing:-1 }}>
              ...
              <span style={{ color:'#BBF7D0' }}>in 10 minutes</span>
            </h1>
            <p style={{ fontSize:16, color:'rgba(255,255,255,0.80)', marginBottom:36, maxWidth:440, lineHeight:1.75 }}>
              KiranaOS connects you with local kirana stores. Order essentials and get them delivered blazing fast by our rider network.
            </p>
          ...
          <div style={{ display:'flex', gap:14, flexWrap:'wrap', justifyContent:'center' }}>
            {[{val:'500+',label:'Kirana Stores'},{val:'10 min',label:'Avg. Delivery'},{val:'50K+',label:'Daily Orders'},{val:'98%',label:'On-time Rate'}].map((s,i) => (
              <div key={i} style={{ background:'rgba(255,255,255,0.12)', borderRadius:18, padding:'22px 30px', textAlign:'center', border:'1px solid rgba(255,255,255,0.18)', backdropFilter:'blur(10px)' }}>
                ...
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.7)', fontWeight:700, marginTop:2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding:'72px 48px', maxWidth:1200, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:52 }}>
          ...
          <h2 style={{ fontSize:36, fontWeight:900, color:'#1C1C1E', marginBottom:8, letterSpacing:-0.5 }}>Join the KiranaOS network</h2>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:24 }}>
          {roles.map(role => (
            <div key={role.id}
              ...
              style={{ background:'#fff', border:`1.5px solid #D1FAE5`, borderRadius:20, padding:32, cursor:'pointer', transition:'all 0.2s ease', position:'relative', overflow:'hidden' }}
              onMouseEnter={e => { e.currentTarget.style.transform='translateY(-5px)'; e.currentTarget.style.boxShadow='0 16px 48px rgba(12,131,31,0.14)'; e.currentTarget.style.borderColor=THEME.accent }}
              onMouseLeave={e => { e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='none'; e.currentTarget.style.borderColor='#D1FAE5' }}
            >
              ...
              <div style={{ position:'relative' }}>
                <div style={{ width:60, height:60, borderRadius:16, background:THEME.gradient, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, marginBottom:20, boxShadow:'0 4px 16px rgba(12,131,31,0.25)' }}>{role.icon}</div>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:THEME.gradient, color:'#fff', borderRadius:12, padding:'13px 18px', fontWeight:800, fontSize:14, boxShadow:'0 4px 14px rgba(12,131,31,0.3)' }}>
      ...
      {/* Footer */}
      <div style={{ textAlign:'center', padding:'28px 48px', borderTop:'1px solid #E8F5E9', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
        ...
        <span style={{ fontWeight:900, color:THEME.primary, fontSize:14 }}>KiranaOS</span>
        <span style={{ color:THEME.accent, fontSize:12, opacity:0.7 }}>· Event-driven delivery · Built for Bharat</span>
      </div>
    </div>
  )
}